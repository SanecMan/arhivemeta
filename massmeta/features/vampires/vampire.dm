...
/datum/antagonist/vampire/on_gain()
	...
	if(ishuman(owner.current))
		...
		var/obj/item/organ/internal/brain/B = H.get_organ_slot(ORGAN_SLOT_BRAIN)
		if(B)
...
/datum/antagonist/vampire/greet()
	...
	if(full_vampire == FALSE)
		if(prob(10))
			owner.current.playsound_local(get_turf(owner.current), 'massmeta/sounds/vampires/lilithspact_alt.ogg',80,0)
		else
			owner.current.playsound_local(get_turf(owner.current), 'massmeta/sounds/vampires/lilithspact.ogg',80,0)
	else
		if(prob(10))
			owner.current.playsound_local(get_turf(owner.current), 'massmeta/sounds/vampires/newvampire_alt.ogg',80,0)
		else
			owner.current.playsound_local(get_turf(owner.current), 'massmeta/sounds/vampires/newvampire.ogg',80,0)
...
/datum/antagonist/vampire/proc/vampire_life(mob/living/source, seconds_per_tick, times_fired)
	...
	if(owner && C.hud_used && C.hud_used.vamp_blood_display)
		...
		C.hud_used.vamp_blood_display.maptext = ANTAG_MAPTEXT(usable_blood, "#DD66DD")
	handle_vampire_cloak()
	if(get_ability(/datum/vampire_passive/regen))
		C.heal_overall_damage(1, 1, 0)
		C.adjustToxLoss(-1, TRUE, TRUE)
		C.adjustOxyLoss(-2.5)

	if(istype(C.loc, /obj/structure/closet/crate/coffin))
		C.heal_overall_damage(4, 4, 0)
...
/datum/antagonist/vampire/get_preview_icon()
	var/icon/vampire_icon = icon('icons/mob/simple/animal.dmi', "bat")
