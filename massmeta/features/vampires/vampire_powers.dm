...
/datum/action/cooldown/spell/pointed/gaze/InterceptClickOn(mob/living/user, params, atom/target_atom)
	...
	switch(protection)
		...
		if(1 to INFINITY)
			...
			return TRUE
		if(INFINITY)
			to_chat(user, span_warning("[T] is blind and is unaffected by your gaze!"))
			return FALSE
	return TRUE
...
/datum/action/cooldown/spell/pointed/hypno/InterceptClickOn(mob/living/user, params, atom/target_atom)
	...
	switch(protection)
		if(1 to INFINITY)
			to_chat(user, span_warning("Your hypnotic powers are dampened by [T]'s eye protection."))
			sleep_duration = 10 SECONDS
		if(INFINITY)
			to_chat(user, span_warning("[T] is blind and is unaffected by hypnosis!"))
			return FALSE
...
/datum/action/cooldown/spell/revive/proc/revive(mob/living/user)
	...
	var/list/missing = carbon_user.get_missing_limbs()
	if(missing.len)
...
/datum/action/cooldown/spell/aoe/screech/cast_on_thing_in_aoe(mob/living/target, mob/living/carbon/user)
	if(!target == user  || !is_vampire(target))
		if(ishuman(target) && target.soundbang_act(1, 0))
			...
			human_target.adjustOrganLoss(ORGAN_SLOT_EARS, 0, 30)
...
/datum/action/cooldown/spell/bats/cast(mob/living/user)
	...
	for(var/direction in GLOB.alldirs) //looking for bat spawns
		...
		if(get_path_to(user, T, /turf/proc/Distance, 1, simulated_only = 0)) // if a path exists, so no dense objects in the way its valid salid
			locs += T
...
/datum/action/cooldown/spell/pointed/vampirize/InterceptClickOn(mob/living/user, params, atom/target_atom)
	...
	if(!QDELETED(user) && !QDELETED(target))
		...
		to_chat(target, span_bolditalic("Come to me, child."))
		sleep(1 SECONDS)
		to_chat(target, span_bolditalic("The world hasn't treated you well, has it?"))
		sleep(1.5 SECONDS)
		to_chat(target, span_bolditalic("Strike fear into their hearts..."))
		...
		target.mind.add_memory("<B>[user] showed you the glory of Lilith. <I>You are not required to obey [user], however, you have gained a respect for them.</I></B>")
		target.Sleeping(600)
